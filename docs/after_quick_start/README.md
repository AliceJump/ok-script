...
- [1. 模板匹配 (Template Matching)](#1-模板匹配-template-matching)
    - [模板 Tab 维护流程（推荐）](#模板-tab-维护流程推荐)
    - [模板 Tab 详细图文流程](#模板-tab-详细图文流程)
...
1. **标注工具**: 使用任何支持导出 COCO 格式的标注工具。推荐使用 **`label-studio==1.15.0`**，因为新版本导出的 COCO
   格式存在兼容性问题。
2. **放置素材**: 将标注工具导出的 `result.json` 文件以及对应的图片文件夹（通常是 `images` 文件夹）完整地放入项目的
   `assets` 目录下。
...
如果你是通过模板项目（含 submodule）维护素材，建议按以下原则操作：

1. **先改模板源，再导出产物**：模板源（例如 `ok_templates/` 子仓库）应作为“源码”，`assets/` 作为运行时产物。
2. **必须使用 Debug 模式维护模板**：运行 `python main_debug.py` 后，在左侧模板 Tab 执行 `Screenshot`、`Markup`、`Save` 流程，并在 Save 弹窗中选择保存到本软件 `assets`。
3. **不要用正式模式做模板维护**：`python main.py` 仅用于正式运行，不建议用于模板素材编辑与导出。
4. **提交时同时检查两处变更**：确认模板源仓库（如子仓库）和主仓库 `assets/` 都有预期更新，避免只提交指针变化或遗漏产物更新。

> **重要**：只有在 `python main_debug.py` 下，模板 Tab 的 Save 弹窗才有“保存到本软件 `assets`”选项。

一句话：先维护模板源，再通过 Debug 模式模板 Tab 保存到 `assets`，最后一起校验并提交。

### 模板 Tab 详细图文流程

详细步骤（含子模块协作、截图/标注/保存顺序、提交流程）见：

- [模板 Tab 详细图文流程（含 submodule 贡献）](./template_tab_submodule_contrib.md)
